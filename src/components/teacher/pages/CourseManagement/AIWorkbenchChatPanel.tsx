// src/components/teacher/pages/CourseManagement/AIWorkbenchChatPanel.tsx
"use client";
...
import styles from './AIWorkbenchChatPanel.module.css';
...
const AIWorkbenchChatPanel: React.FC = () => {
    const initialMessages: Message[] = [
        { 
          id: 'init-assistant-1', 
          role: 'assistant', 
          content: "你好！我是EduSpark智能助教。无论是解答课程疑问，还是生成练习题，我都在这里为你服务。试试问我：“请用Python写一个快速排序”或“帮我出5道关于牛顿第二定律的选择题”。", 
          isComplete: true 
        }
      ];
          ...
          >
            {msg.role === 'assistant' && (msg.isThinking || msg.isComplete) && (
              <div className={styles.assistantMsgHeader}>
                ...
                {msg.isComplete && (
                  <span className={styles.statusTagComplete} onClick={msg.thinkingText ? () => toggleThinkingPanel(msg.id) : undefined} style={{cursor: msg.thinkingText ? 'pointer' : 'default'}}>
                    <i className={`fas ${msg.thinkingText ? `fa-chevron-right ${showThinkingPanelId === msg.id ? styles.chevronOpen : ''}` : 'fa-check-circle'}`} style={{ marginRight: '8px', transition: 'transform 0.2s', color: msg.thinkingText ? undefined : '#52c41a' }}></i>
                    回答完毕{msg.thinkingText ? ` (点击${showThinkingPanelId === msg.id ? '收起' : '展开'}思考过程)` : ''}
                  </span>
                )}
              </div>
            )}
            ...
            <div className={`${styles.messageContent} markdown-body`}>
              <ReactMarkdown
                ...
                components={{
                    ...
                    if (match && match[1] === 'mermaid') return <pre className="mermaid">{String(children)}</pre>;
                    return !inline && match ? (
            ...
            {msg.role === 'assistant' && msg.isComplete && msg.id !== 'init-assistant-1' && (
...
export default AIWorkbenchChatPanel;